...
namespace FeedbackService.Facade
{
    public class CustomerFacade : BaseFacade, ICustomerFacade
    {
        ...
        public async Task<Customer> GetCustommerByIdAsync(long userId, CancellationToken cancellationToken)
        {
            ...
            var cachedCustomers = await GetFromCacheAsync<Customer>(typeName, userId.ToString(), CustomerErrorMessages.UnableToRetrieveCustomer, cancellationToken);
            Customer customer = default;

            //If the user is already in cache, we just return true
            if (cachedCustomers != null)
            {
                customer = cachedCustomers.Where(customer => customer.Sid == userId).FirstOrDefault();
                if (customer != null)
                {
                    return customer;
                }
