...
namespace FeedbackService.Managers
{
    public class BaseManager
    {
        ...
        public BaseManager(IDistributedCacheManager distributedCacheManager, IOptions<CacheOptions> cacheOptions)
        {
        ...
        protected virtual async Task<List<T>> GetFromCacheAsync<T>(string typeName, string entityId, string exceptionMessage, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.Error(ex, exceptionMessage, entityId, _distributedCacheManager);
            }
